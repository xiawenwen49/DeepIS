...
from .utils import MixedDropout, sparse_matrix_to_torch, to_torch


def calc_A_hat(adj_matrix: sp.spmatrix) -> sp.spmatrix:
    nnodes = adj_matrix.shape[0]
    A = adj_matrix + sp.eye(nnodes) # add self loops
    D_vec = np.sum(A, axis=1).A1 # flattened
...
class PPRPowerIteration(nn.Module):
    def __init__(self, adj_matrix: sp.spmatrix, alpha: float, niter: int, drop_prob: float = None):
        ...
        M = calc_A_hat(adj_matrix)
        # M = adj_matrix # prob_matrix
        self.register_buffer('A_hat', to_torch((1 - self.alpha) * M)) # (1 - alpha) is here. Ndarray, NOT sparse matrix now.
